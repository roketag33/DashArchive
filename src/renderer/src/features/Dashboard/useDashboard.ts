...
export function useDashboard(): UseDashboardReturn {
  ...
  // Organization Logic
  const handleSortFolder = useCallback(
    ...
    [t]
  ) // useCallback for handleSortFolder to prevent infinite loop
  ...
  useEffect(() => {
    const handleFileAdded = (payload: string | { filePath: string; folderId: string }): void => {
      ...
      if (
        selectedFolder &&
        (folderId === selectedFolder.id || (!folderId && path.startsWith(selectedFolder.path)))
      ) {
  ...
  const handleSaveFolderRules = async (
    folderId: string,
    ruleIds: string[],
    scanFrequency?: string
  ): Promise<void> => {
    try {
      ...
      if (scanFrequency !== undefined) {
        await window.api.updateFolder(folderId, { scanFrequency })
      }
      await loadFolders() // Refresh to get updated frequency in UI
