...
class AIService {
  ...
  public async classify(text: string, labels: string[]): Promise<string> {
    ...
    try {
      ...
      const result = await this.classifier(text.slice(0, 400), candidateLabels) // Truncate to ~100 tokens max
  ...
  public async suggestCategories(filePaths: string[]): Promise<string[]> {
    ...
    for (const filePath of samples) {
      try {
        ...
        const result = await this.classifier(text.slice(0, 400), COMMON_CATEGORIES)
